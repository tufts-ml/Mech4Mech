...
from utils import flatten_params, inv_softplus, use_posterior, add_variational_layers, plot_losses
...
def main():
    ...
    #num_backbone_params = 308000000 #Add an additional 308 million params to represent the number of params in gemmaembed LLM.
    ...
    lr_list = [0.5, 0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001] #List of learning rates. Older rates: 0.001, 0.0001, 0.00001, 0.000001
    num_epochs = 1000 #Number of epochs. 
    seed = 123 #Seed to set initial random model parameters. 
    best_loss = float('inf') 
    ...
    for lr in lr_list: 
        ...
        plot_losses(num_epochs, loss_list, "loss", lr)
        plot_losses(num_epochs, kl_list, "kl", lr)
        plot_losses(num_epochs, nll_list, "nll", lr)

        evidence_dict[lr] = loss["loss"].item()
        kL_dict[lr] = loss["kl"].item()
        nll_dict[lr] = loss["nll"].item()
